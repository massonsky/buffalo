...
func (g *PythonPydanticGenerator) GenerateBaseModel(opts GenerateOptions) (GeneratedFile, error) {
	...
	if g.isV2() {
		...
		b.WriteString("        )\n")
		b.WriteString("        # Рекурсивно убираем поля базового класса (id, timestamps)\n")
		b.WriteString("        # на всех уровнях вложенности — вложенные модели тоже\n")
		b.WriteString("        # наследуют BaseModel и содержат эти поля.\n")
		b.WriteString("        dict_obj = self._strip_base_fields(dict_obj)\n")
		...
		b.WriteString("        dict_obj = self._strip_base_fields(\n")
		b.WriteString("            self.dict(exclude_none=True, by_alias=True)\n")
		b.WriteString("        )\n")
		b.WriteString("        return ParseDict(dict_obj, target_proto_class())\n")
	}
