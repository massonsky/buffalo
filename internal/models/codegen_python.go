...
func (g *PythonPydanticGenerator) GenerateBaseModel(opts GenerateOptions) (GeneratedFile, error) {
	...
	if g.isV2() {
		...
		b.WriteString("                use_integers_for_enums=True,\n")
		b.WriteString("            )\n")
		b.WriteString("        # Убираем поля базового класса — их нет в proto-схеме\n")
		b.WriteString("        filtered = {k: v for k, v in dict_obj.items() if k not in cls._base_model_fields}\n")
		b.WriteString("        return cls.model_validate(filtered)\n\n")
		...
		b.WriteString("        # exclude — убираем поля базового класса (id, timestamps),\n")
		b.WriteString("        # которых нет в proto-схеме.\n")
		...
		b.WriteString("                use_integers_for_enums=True,\n")
		b.WriteString("            )\n")
		b.WriteString("        # Remove base model fields — they are not in the proto schema\n")
		b.WriteString("        filtered = {k: v for k, v in dict_obj.items() if k not in cls._base_model_fields}\n")
		b.WriteString("        return cls.parse_obj(filtered)\n\n")
		b.WriteString("    def to_proto(self, proto_class: Type[T] | None = None) -> Any:\n")
		b.WriteString("        target_proto_class = proto_class or self.proto_class\n")
		b.WriteString("        if target_proto_class is None:\n")
		b.WriteString("            raise NotImplementedError(\n")
		b.WriteString("                \"Subclasses must define proto_class or pass proto_class to to_proto\"\n")
		b.WriteString("            )\n")
		b.WriteString("        # Exclude base model fields (id, timestamps) not present in proto schema\n")
		b.WriteString("        dict_obj = self.dict(exclude_none=True, by_alias=True, exclude=self._base_model_fields)\n")
		b.WriteString("        return ParseDict(dict_obj, target_proto_class())\n")
	}
