...
_RUST_TAG = tag_class(
    attrs = {
        ...
        "protoc_gen_prost": attr.label(
            ...
            doc = "Binary label for protoc-gen-prost. Defaults to the crate_universe repository bundled by rules_buffalo.",
        ),
        "protoc_gen_tonic": attr.label(
            default = Label("@buffalo_rust_plugins//:protoc-gen-tonic__protoc-gen-tonic"),
            allow_single_file = True,
            doc = "Binary label for protoc-gen-tonic. Defaults to the crate_universe repository bundled by rules_buffalo.",
        ),
    },
    doc = "Enables the hermetic Rust prost/tonic plugin binaries bundled by rules_buffalo.",
)
...
def _select_config(module_ctx):
    cfg = struct(
        ...
        protoc_gen_prost = None,
        protoc_gen_tonic = None,
        ...
        for tag in mod.tags.rust:
            if mod.is_root or not seen_root_rust:
                cfg = struct(
                    ...
                    protoc_gen_prost = tag.protoc_gen_prost,
                    protoc_gen_tonic = tag.protoc_gen_tonic,
                    enable_typescript = cfg.enable_typescript,
                    node_version = cfg.node_version,
                    ts_proto_version = cfg.ts_proto_version,
                )
...
buffalo = module_extension(
    ...
    doc = "Provisions the hermetic Buffalo toolchain. Use buffalo.toolchain() to pin versions, buffalo.rust() to enable bundled Rust prost/tonic plugins, and buffalo.typescript() to enable the ts-proto plugin (Node.js bundled).",
)
