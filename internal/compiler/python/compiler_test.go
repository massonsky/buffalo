...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Fix imports with preserveStructure=true to test full path scenario
			if err := c.fixFileImports(pyFile, tc.modulePrefix, "", true, nil); err != nil {
...
func TestValidateWorkDir(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "multiple_dots",
			workDir: "my_app.api.v1",
			wantErr: false,
		},
		{
			name:    "with_underscore",
			workDir: "_private.module_name",
			wantErr: false,
		},
		{
			name:    "with_numbers",
			workDir: "app123.module456",
			wantErr: false,
		},
		{
...
func TestCompiler_FixImports_WithWorkDir(t *testing.T) {
	log := logger.New()
	opts := &Options{
		WorkDir: "myproject",
	}
	...
	tempDir := t.TempDir()
	outputDir := filepath.Join(tempDir, "generated", "python")
...
class MyServiceStub:
    pass
`
	grpcFile := filepath.Join(outputDir, "service_pb2_grpc.py")
	if err := os.WriteFile(grpcFile, []byte(grpcContent), 0644); err != nil {
		t.Fatalf("Failed to create grpc file: %v", err)
	}
	...
	// Check that the grpc file was fixed with workdir prefix
	result, err := os.ReadFile(grpcFile)
	if err != nil {
		t.Fatalf("Failed to read result file: %v", err)
	}

	// With workdir="myproject", outputDir contributes to the import path, so:
	// from myproject.generated.python import service_pb2
	expectedImport := "from myproject.generated.python import service_pb2 as service__pb2"
	if !contains(string(result), expectedImport) {
		t.Errorf("Expected import with workdir prefix.\nExpected to contain:\n%s\nGot:\n%s", expectedImport, string(result))
	}
}

func TestCompiler_FixImports_WithWorkDir_PreserveStructure(t *testing.T) {
	log := logger.New()
	opts := &Options{
		WorkDir: "myapp.api",
	}
...
`
	...
	// Check that the grpc file was fixed with workdir prefix and full structure
	result, err := os.ReadFile(grpcFile)
	if err != nil {
		t.Fatalf("Failed to read result file: %v", err)
	}

	// With workdir="myapp.api", outputDir contributes to the import path, so:
	// from myapp.api.generated.python.module1.v1 import service_pb2
	expectedImport := "from myapp.api.generated.python.module1.v1 import service_pb2 as service__pb2"
	if !contains(string(result), expectedImport) {
		t.Errorf("Expected import with workdir prefix and full structure.\nExpected to contain:\n%s\nGot:\n%s", expectedImport, string(result))
	}
}

func TestCompiler_FixImports_WithWorkDir_MatchesGeneratedPythonStructure(t *testing.T) {
	log := logger.New()
	opts := &Options{
		WorkDir: "araviec_apis",
	}
	c := New(log, opts)

	tempDir := t.TempDir()
	outputDir := filepath.Join(tempDir, "generated", "python")
	moduleDir := filepath.Join(outputDir, "ais", "v1")
	...
	grpcContent := `# Generated by protoc-gen-grpc
import grpc
from ais.v1 import ais_service_pb2 as ais_dot_v1_dot_ais__service__pb2
from araviec.telemetry.v1 import ais_telemetry_pb2 as araviec_dot_telemetry_dot_v1_dot_ais__telemetry__pb2
from araviec_apis.generated.python.araviec.telemetry.v1 import existing_pb2 as existing__pb2

class AisServiceStub:
    pass
`
	grpcFile := filepath.Join(moduleDir, "ais_service_pb2_grpc.py")
	if err := os.WriteFile(grpcFile, []byte(grpcContent), 0644); err != nil {
		t.Fatalf("Failed to create grpc file: %v", err)
	}
	...
	compileOpts := compiler.CompileOptions{
		OutputDir:              outputDir,
		ProtoPaths:             []string{filepath.Join(tempDir, "araviec")},
		PreserveProtoStructure: true,
		ProjectDir:             tempDir,
	}
	if err := c.fixImports(compileOpts, []string{grpcFile}); err != nil {
		t.Fatalf("fixImports failed: %v", err)
	}
	...
	expectedService := "from araviec_apis.generated.python.ais.v1 import ais_service_pb2 as ais_dot_v1_dot_ais__service__pb2"
	if !contains(resultStr, expectedService) {
		t.Errorf("Expected service import to match generated/python structure.\nExpected to contain:\n%s\nGot:\n%s", expectedService, resultStr)
	}

	expectedTelemetry := "from araviec_apis.generated.python.telemetry.v1 import ais_telemetry_pb2 as araviec_dot_telemetry_dot_v1_dot_ais__telemetry__pb2"
	if !contains(resultStr, expectedTelemetry) {
		t.Errorf("Expected telemetry import to match generated/python structure.\nExpected to contain:\n%s\nGot:\n%s", expectedTelemetry, resultStr)
	}

	expectedExisting := "from araviec_apis.generated.python.telemetry.v1 import existing_pb2 as existing__pb2"
	if !contains(resultStr, expectedExisting) {
		t.Errorf("Expected already-prefixed import to be corrected.\nExpected to contain:\n%s\nGot:\n%s", expectedExisting, resultStr)
	}
}

func TestCompiler_FixImports_SkipsExternalDependencies(t *testing.T) {
	log := logger.New()
	c := New(log, nil)
	...
	// Create a _pb2_grpc.py file with both internal and external imports
	grpcContent := `# Generated by protoc-gen-grpc
import grpc
from google.protobuf import descriptor_pb2
from google.protobuf import empty_pb2
from google.api import annotations_pb2
from grpc import status_pb2
from module1.v1 import service_pb2 as service__pb2
...
`
	...
	// External dependencies should NOT be modified
	if !contains(resultStr, "from google.protobuf import descriptor_pb2") {
		t.Errorf("google.protobuf import was incorrectly modified:\n%s", resultStr)
	}
	if !contains(resultStr, "from google.protobuf import empty_pb2") {
		t.Errorf("google.protobuf empty import was incorrectly modified:\n%s", resultStr)
	}
	if !contains(resultStr, "from google.api import annotations_pb2") {
		t.Errorf("google.api import was incorrectly modified:\n%s", resultStr)
	}

	// Internal imports SHOULD be modified (flat structure, so just output dir prefix)
	if !contains(resultStr, "from generated.python import service_pb2 as service__pb2") {
		t.Errorf("Internal module1.v1 import was not fixed correctly:\n%s", resultStr)
	}
	if !contains(resultStr, "from generated.python import user_pb2") {
		t.Errorf("Internal user.v1 import was not fixed correctly:\n%s", resultStr)
	}
}
